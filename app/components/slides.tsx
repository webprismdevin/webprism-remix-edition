...
export default function Slides({ slides }: { slides: any }) {
    ...
    return (
        <motion.div
            className="relative h-[700px] border-b-4 border-white border-t-4 bg-black"
            custom={direction}
            key={page}
        >
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    exit={{ x: 800 * direction, opacity: 0, transition: { duration: 0.2 } }}
                    custom={direction}
                    key={page}
                    className={`relative grid h-full w-full grid-cols-1 bg-white ${slides[index].image2 ? 'lg:grid-cols-2' : ''
                        }`}
                >
                    <div className="absolute z-10 grid h-full w-full place-items-center px-12">
                        <div className="text-center">
                            ...
                            {slides[index].cta?.to && <Link to={slides[index].cta?.to}>
                                {slides[index].cta.text}
                            </Link>}
                        </div>
                    </div>
                    <div
                        className={`h-full min-w-full max-w-full overflow-hidden ${slides[index].image2 ? 'lg:max-w-1/2' : ''
                            }`}
                    >
                        {slides[index].image && <img
                            ...
                            className={`min-h-full min-w-full object-cover ${slides[index].image2 ? '' : 'lg:min-w-full'
                                }`}
